...
test('call queue exposes recording and voicemail playback for missed calls', function () {
    ...
    $response
        ...
        ->assertJsonPath('items.0.has_voicemail', true)
        ->assertJsonPath('items.0.show_play_recording_action', false)
        ->assertJsonPath('items.0.show_play_voicemail_action', true)
        ->assertJsonPath('items.0.dropdown_label', 'Call · Missed · Voicemail · Voicemail Caller');
...
test('call queue shows recording and voicemail when they are different files', function () {
    ...
    $this->actingAs($advisor)
        ->getJson(route('operations.telephony.call-queue'))
        ->assertOk()
        ->assertJsonPath('items.0.has_recording', true)
        ->assertJsonPath('items.0.has_voicemail', true)
        ->assertJsonPath('items.0.show_play_recording_action', true)
        ->assertJsonPath('items.0.show_play_voicemail_action', true);
});
...
test('call queue returns unworked recent sessions for all advisors', function () {
    ...
    $this->actingAs($advisor)
        ...
        ->assertJsonPath('calls.0.context_summary', 'RO Awaiting Approval');
});
...
test('call queue open ros url targets open repair orders when customer has multiple active jobs', function () {
    ...
    $this->actingAs($advisor)
        ->getJson(route('operations.telephony.call-queue'))
        ->assertOk()
        ->assertJsonPath('calls.0.context_summary', '2 Open ROs')
...
test('call queue api includes unread inbound sms in items and summary', function () {
    ...
    $this->post(route('webhooks.communications.twilio.messaging.incoming'), [
        'MessageSid' => 'SMcallqueue001',
        'From' => '+17195551234',
        'To' => '+17195559999',
        'Body' => 'Is my vehicle ready?',
        'NumMedia' => '0',
    ])->assertOk();
...
test('operations layout bootstraps comms queue with unread sms', function () {
    ...
    $this->post(route('webhooks.communications.twilio.messaging.incoming'), [
        'MessageSid' => 'SMlayout001',
        'From' => '+17195551234',
        'To' => '+17195559999',
        'Body' => 'Where is my car?',
        'NumMedia' => '0',
    ])->assertOk();

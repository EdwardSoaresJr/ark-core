...
test('admins can open all staff learn sections', function () {
    ...
    $this->get(route('operations.learn.show', ['role' => 'admin', 'article' => 'telephony-sip-setup']))
        ->assertOk()
        ->assertSee('Telephony and SIP desk phones')
        ->assertSee('not a PBX')
...
test('admins can read owner excellence guides and default learn entry opens owner track', function () {
    ...
    $this->get(route('operations.learn.show', ['role' => 'owner', 'article' => 'square-payments-setup']))
        ->assertOk()
        ->assertSee('Square payments setup')
        ->assertSee('Square credentials (Settings)')
        ->assertSee('encrypted in the shop database')
        ->assertSee(route('webhooks.square'), false)
        ->assertSee('Terminal device ID')
        ->assertSee('Connected / Terminal API mode')
        ->assertSee('Switch mode')
        ->assertSee('Device Code')
        ->assertSee('CreateDeviceCode')
        ->assertSee('Generate pairing code')
        ->assertSee('full reset')
        ->assertSee('Save Square settings')
        ->assertSee('5 minutes')
        ->assertSee('merchant not authorized for device_id')
        ->assertSee('integrate-square-terminal', false);
});

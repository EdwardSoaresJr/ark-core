...
test('portal estimate shows financing apply options', function () {
    $repairOrder = estimateLinkRepairOrder();
    ...
    $this->get(route('portal.estimates.show', ['token' => $plainToken]))
        ...
        ->assertSee('public-financing-note--featured', false)
        ->assertSee('public-wisetack-prequal-button', false)
        ->assertSee('Prequalify now', false)
        ->assertSee(\App\Ark\Operations\Leads\Public\PublicSurfaceSettings::WISETACK_PREQUAL_URL, false)
        ->assertSee(\App\Ark\Operations\Leads\Public\SynchronyCarCareUrls::APPLY_BUTTON_IMAGE, false)
        ->assertSee(\App\Ark\Operations\Leads\Public\SynchronyCarCareUrls::embedUrl(), false)
        ->assertSee('Financing options →', false);
});

...
test('estimate follow-up lifecycle generates call recommendation when estimate viewed repeatedly', function () {
    ...
    $this->actingAs($advisor)
        ...
        ->assertOk()
        ->assertSee('Call Jason')
        ->assertSee('Estimate viewed 3×')
        ->assertSee('Increase approval likelihood.');
});
...
test('parts arrival lifecycle surfaces receive recommendation for ordered parts', function () {
    ...
    $this->actingAs($advisor)
        ->get(route('operations.today'))
        ->assertOk()
        ->assertSee('Parts arrived')
        ->assertSee('2019 Toyota Tacoma')
        ->assertSee('Technician can resume work.');
});
...
test('today projection excludes completed estimate follow-up recommendations', function () {
    ...
    $titles = collect($projection->nonEmptySections())
        ->flatMap(fn ($section) => $section->actions)
        ->pluck('title');
    ...
    $after = app(TodayProjectionBuilder::class)->forUser($advisor);
    $afterTitles = collect($after->nonEmptySections())
        ->flatMap(fn ($section) => $section->actions)
        ->pluck('title');

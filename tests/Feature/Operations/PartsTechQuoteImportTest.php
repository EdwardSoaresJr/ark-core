...
use App\Ark\Operations\Parts\PartsTechShopSessionLockedException;
...
test('parts tech quote preview keeps shop default parts matrix when multiple scopes exist', function () {
    config()->set('services.partstech.username', 'ark-shop');
    config()->set('services.partstech.password', 'shop-password');
    config()->set('services.partstech.base_url', 'https://partstech.test');
    ...
    RepairOrderConcern::query()->create([
        ...
        'summary' => 'Cooling',
        'position' => 2,
        'billing_posture' => ConcernBillingPosture::Default,
    ]);

    $matrices = ShopSettings::DEFAULT_PARTS_MATRICES;
    $matrices[1]['name'] = 'Parts';

    ShopSettings::current()->update([
        'parts_matrices' => $matrices,
    ]);

    expect(collect(ShopSettings::current()->partsMatrices())->pluck('name')->all())
        ->toBe(['Filters', 'Fluids', 'OEM Parts', 'Parts', 'Warranty (No Markup)']);
    ...
    partsTechQuoteImportGraphqlFake('R88042', [
        [
            ...
            'quantity' => 1,
            'partNumber' => 'MO339',
            'partName' => 'Mopar Engine Oil Filter',
            'brand' => ['name' => 'Mopar'],
            'builtItem' => ['product' => ['price' => 11.89]],
        ],
    ]);
    ...
    $this->actingAs(actingAsLearnCurrentAdvisor())
        ...
        ->assertJsonPath('default_repair_order_concern_id', null)
        ->assertJsonPath('default_parts_matrix_key', 'aft-parts')
        ->assertJsonPath('default_parts_matrix_name', 'Parts');
});

test('parts tech quote import uses the matrix shown on the row', function () {
    config()->set('services.partstech.username', 'ark-shop');
    config()->set('services.partstech.password', 'shop-password');
    config()->set('services.partstech.base_url', 'https://partstech.test');
    ...
    expect($line->pricing_matrix_key)->toBe('aft-parts')
        ->and($line->pricing_matrix_name)->toBe('AFT Parts')
        ->and($line->part_cost_cents)->toBe(1189)
        ->and($line->unit_price_cents)->toBe(2616);
});
...
test('parts tech quote pull claims an unnumbered cart with parts instead of replacing it', function () {
    config()->set('services.partstech.username', 'ark-shop');
    config()->set('services.partstech.password', 'shop-password');
    config()->set('services.partstech.base_url', 'https://partstech.test');
    ...
    partsTechQuoteImportGraphqlFake('', [
        [
            'id' => 'item-1',
            'quantity' => 1,
            'partNumber' => '22401-1KT0A',
            'partName' => 'Spark Plug',
            'brand' => ['name' => 'NGK'],
            'builtItem' => ['product' => ['price' => 8.50]],
        ],
    ]);
    ...
    $this->actingAs(actingAsLearnCurrentAdvisor())
        ->getJson(route('operations.repair-orders.partstech.import.preview', $repairOrder))
        ->assertOk()
        ->assertJsonPath('lines.0.part_number', '22401-1KT0A');

    Http::assertNotSent(function ($request): bool {
        $query = (string) (json_decode((string) $request->body(), true)['query'] ?? '');

        return str_contains($query, 'archiveCart');
    });
    Http::assertNotSent(function ($request): bool {
        $query = (string) (json_decode((string) $request->body(), true)['query'] ?? '');

        return str_contains($query, 'createCart');
    });
});
...
test('parts tech catalog prepare retags a leftover foreign cart instead of locking', function () {
    config()->set('services.partstech.username', 'ark-shop');
    config()->set('services.partstech.password', 'shop-password');
    config()->set('services.partstech.base_url', 'https://partstech.test');
    ...
    Http::fake([
        ...
        'partstech.test/graphql' => function ($request) {
            ...
            if (str_contains($query, 'updateCart')) {
                return Http::response(['data' => ['updateCart' => ['cart' => ['id' => 'cart-busy']]]], 200);
            }

            if (str_contains($query, 'activateCart')) {
                return Http::response(['data' => ['activateCart' => ['cart' => ['id' => 'cart-busy', 'active' => true]]]], 200);
            }

            if (str_contains($query, 'updateActiveCartPurchaseOrderNumber')) {
                return Http::response(['data' => ['updateActiveCartPurchaseOrderNumber' => [
                    '__typename' => 'UpdateCartPurchaseOrderNumberSuccessPayload',
                    'updatedCartId' => 'cart-busy',
                ]]], 200);
            }
            ...
            if (str_contains($query, 'vehicles')) {
                return Http::response(['data' => ['vehicles' => [['id' => 'vehicle-1', 'vin' => '1HGCM82633A004352']]]], 200);
            }

            if (str_contains($query, 'linkVehicleToCart')) {
                return Http::response(['data' => ['linkVehicleToCart' => ['cart' => ['id' => 'cart-busy']]]], 200);
            }
    ...
    $this->actingAs(actingAsLearnCurrentAdvisor())
        ...
        ->assertJsonMissingPath('cart_locked')
        ->assertJsonPath('warnings.0', 'Cart still has leftover parts from R12. They are now tagged to this repair order. Clear them in PartsTech if they do not belong here.');

    Http::assertNotSent(fn ($request): bool => str_contains((string) data_get(json_decode($request->body(), true), 'query', ''), 'archiveCart'));
    Http::assertSent(function ($request) use ($partsTechReference): bool {
        $payload = json_decode($request->body(), true);
        $query = (string) data_get($payload, 'query', '');

        return str_contains($query, 'updateCart')
            && ($payload['variables']['input']['repairOrderNumber'] ?? null) === $partsTechReference;
    });
});
...
test('parts tech quote preview refuses sync when another repair order owns the active cart with parts', function () {
    ...
    expect(fn () => $preparer->prepareOrReport($repairOrder, syncOnly: true))
        ->toThrow(PartsTechShopSessionLockedException::class);
});

...
it('renders briefing empty state with yesterday summary', function (): void {
    ...
    briefingGet($this, $advisor)
        ->assertOk()
        ->assertSee('Good morning, Edward.')
        ->assertSee('Yesterday')
        ->assertSee('Revenue')
        ->assertSee('Completed repair orders')
        ->assertSee('Approval rate')
        ->assertSee('You\'re caught up')
        ->assertSee('Continue scheduled work')
...
it('surfaces estimate follow-up when estimate viewed repeatedly without follow-up', function (): void {
    ...
    briefingGet($this, $advisor)
        ->assertOk()
        ->assertSee('Follow up estimate')
        ->assertSee('Estimate viewed 3× without follow-up');
});

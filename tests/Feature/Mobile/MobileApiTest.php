...
test('advisor can load mobile me and notifications polling surface', function (): void {
    ...
    $this->withToken($token)
        ...
        ->assertJsonPath('theme.display_mode', 'system')
...
test('mobile me and conversation call actions use shop callback when twilio is ready', function (): void {
    ...
    $this->withToken($token)
        ->getJson('/api/mobile/me')
        ->assertOk()
        ->assertJsonPath('telephony.dial_method', 'shop_callback');
    ...
    expect($callAction['label'])->toBe('Callback')
        ->and($callAction['params']['dial_method'])->toBe('shop_callback')
...
test('advisor mobile square terminal payment completes after poll', function (): void {
    ...
    $fakeSquare = new \App\Ark\Operations\Payments\FakeSquarePaymentsClient;
    $this->app->instance(\App\Ark\Operations\Payments\FakeSquarePaymentsClient::class, $fakeSquare);
    $this->app->bind(\App\Ark\Operations\Payments\Contracts\SquarePaymentsClient::class, fn () => $fakeSquare);
    ...
    $initiate = $this->withToken($token)
        ->postJson('/api/mobile/repair-orders/'.$repairOrder->repair_order_id.'/square-payments', [
            'capture_surface' => \App\Ark\Operations\Payments\PaymentCaptureSurface::Terminal->value,
        ])
        ->assertOk();
    ...
    $this->withToken($token)
        ->getJson('/api/mobile/repair-orders/'.$repairOrder->repair_order_id.'/square-payments/'.$attemptId)
        ->assertOk()
        ->assertJsonPath('attempt.status', \App\Ark\Operations\Payments\PaymentGatewayAttemptStatus::Completed->value);

    $this->withToken($token)
        ->getJson('/api/mobile/repair-orders/'.$repairOrder->repair_order_id)
        ->assertOk()
        ->assertJsonPath('workspace.square_payment', null);
});
...
test('advisor mobile square terminal deposit completes after poll', function (): void {
    config()->set('services.square.application_id', 'sq0idp-test-app');
    config()->set('services.square.access_token', 'test-token');
    config()->set('services.square.location_id', 'LOC123');
    config()->set('services.square.webhook_signature_key', 'test-signature-key');
    ...
    $repairOrder = repairOrderWithSuggestedPartDeposit(\App\Ark\Operations\RepairOrders\RepairOrderStatus::Approved);

    $initiate = $this->withToken($token)
        ->postJson('/api/mobile/repair-orders/'.$repairOrder->repair_order_id.'/square-deposits', [
            'capture_surface' => \App\Ark\Operations\Payments\PaymentCaptureSurface::Terminal->value,
            'amount' => '50.00',
        ])
        ->assertOk();
...
test('technician can load inspection item living record with evidence and recommendation hint', function (): void {
    ...
    $this->withToken($token)
        ...
        ->assertOk()
        ->assertJsonPath('item.status_display', 'Replace')

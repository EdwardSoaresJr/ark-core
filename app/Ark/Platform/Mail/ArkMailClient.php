...
final class ArkMailClient
{
     ...
     */
    public function sendTransactional(array $payload): array
    {
        return $this->request('POST', '/api/v1/services/mail/messages/transactional', $payload, timeout: 30);
    }
    ...
    private function request(string $method, string $path, ?array $body = null, int $timeout = 20): array
    {
        ...
        try {
            ...
            return [
                ...
                'message' => 'Shop email is unavailable.',
            ];
        }
        ...
        return array_merge([
            ...
            'message' => is_string($json['message'] ?? null) ? $json['message'] : 'Shop email was rejected.',
        ], $json);
    }
}

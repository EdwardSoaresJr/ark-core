...
class RepairOrderStatusCatalogSettingsController
{
    public function __invoke(Request $request, RepairOrderStatusCatalogUpdater $updater): RedirectResponse
    {
        $data = $request->validate([
            ...
            'create.mileage_requirement' => ['nullable', 'string', Rule::in(['none', 'in', 'out', 'both'])],
            'create.is_terminal' => ['nullable'],
            ...
            'statuses.*.requires_mileage_in' => ['nullable'],
            'statuses.*.requires_mileage_out' => ['nullable'],
            'statuses.*.mileage_requirement' => ['nullable', 'string', Rule::in(['none', 'in', 'out', 'both'])],
            'statuses.*.is_terminal' => ['nullable'],
            'statuses.*.sort_order' => ['nullable', 'integer', 'min:0', 'max:999'],

...
use App\Ark\Operations\Growth\DispatchRepairOrderClosedForGrowth;
use App\Models\User;
...
final class RepairOrderPosting
{
    public function __construct(
        ...
        private readonly DispatchRepairOrderClosedForGrowth $growthDispatch,
    ) {}
    ...
    private function applyPost(RepairOrder $repairOrder, ?User $actor = null): RepairOrder
    {
        ...
        $repairOrder = $repairOrder->refresh();

        try {
            $this->growthDispatch->dispatch($repairOrder);
        } catch (\Throwable $exception) {
            Log::warning('Growth attribution dispatch failed after repair order post.', [

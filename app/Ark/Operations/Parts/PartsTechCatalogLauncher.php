...
/**
 * Opens PartsTech in the browser. When shop password credentials are configured,
 * ARK prepares the active cart via GraphQL (repair order number + VIN or YMM) before redirect.
 */
final class PartsTechCatalogLauncher
{
    ...
    public function launchUrl(RepairOrder $repairOrder, ?int $concernId = null): ?string
    {
        ...
        $query = array_filter([
            ...
            // PartsTech catalog UI binds the PO field from poNumber; GraphQL carts use repairOrderNumber.
            'poNumber' => $partsTechReference,
            'purchaseOrderNumber' => $partsTechReference,
            'repairOrderNumber' => $partsTechReference,
            'concern_id' => $concernId,
            // Bust SPA reuse when the advisor reopens PartsTech in the same named browser tab.

<div class="ops-learn-prose">
    ...
    <p>Portal payment links pay <strong>issued invoice balance</strong> — not draft estimates. Estimate deposit after portal authorization uses a separate Square flow on the customer portal when portal pay is enabled — see <a href="{{ route('operations.learn.show', ['role' => 'advisor', 'article' => 'deposits-and-invoicing']) }}">Deposits and invoicing</a>.</p>
    ...
    <h3>When to send</h3>
    <p>Customer picked up already but calls to pay — send link, confirm receipt in thread, mark RO payment when webhook completes.</p>
    ...
    <p>Square webhook drives authoritative paid state — if customer insists they paid but RO shows due, check attempt status before arguing.</p>
    <p>Owner setup: <a href="{{ route('operations.learn.show', ['role' => 'owner', 'article' => 'square-payments-setup']) }}">Square payments setup</a>.</p>
    ...
    <p>Counter card: <a href="{{ route('operations.learn.show', ['role' => 'advisor', 'article' => 'square-terminal-floor']) }}">Square Terminal on the floor</a>.</p>

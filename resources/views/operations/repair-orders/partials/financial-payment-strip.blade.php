...
<div id="financial-payment-panel" class="ops-financial-payment-strip {{ $tone }}">
    ...
    @can(App\Ark\Runtime\Authorization\ArkCapability::RepairOrdersCloseout->value)
        ...
        @include('operations.repair-orders.partials.financial-square-payments', [
            'repairOrder' => $repairOrder,
            'financial' => $financial,
            'estimateVersion' => $estimateVersion,
            'squareIntent' => 'payment',
        ])

        @if ($financial['canRecordDeposit'] ?? false)
            @if ($financial['canChargeDepositWithSquare'] ?? false)
                @include('operations.repair-orders.partials.financial-square-payments', [
                    'repairOrder' => $repairOrder,
                    'financial' => $financial,
                    'estimateVersion' => $estimateVersion,
                    'squareIntent' => 'deposit',
                ])
            @endif

@php
    ...
    $squareEnvironment = old('square_environment', $settings->square_environment ?? 'sandbox');
    $squareIsSandbox = $squareEnvironment === 'sandbox';
@endphp
...
<section x-show="active === 'payments'" x-cloak class="space-y-3">
    <div class="border border-slate-300 bg-white p-4">
        <div>
            ...
            @if ($platformPaymentsCapture)
                <p class="mt-1 text-xs leading-5 text-slate-500">
                    Card capture for this shop is managed by ARK Platform. Credentials, environment, webhooks, and reader pairing are not stored or configured here.
                </p>
            @else
                <p class="mt-1 text-xs leading-5 text-slate-500">
        ...
        @if (! $platformPaymentsCapture && is_array($squarePairing))
            <div
    ...
    @unless ($platformPaymentsCapture)

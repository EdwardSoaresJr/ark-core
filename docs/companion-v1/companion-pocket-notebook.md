# Companion Pocket Notebook

**Status:** Living observation log — Era 4  
**Frozen:** Mission · hierarchy · doctrine · navigation · product grammar ([`MISSION.md`](MISSION.md))  
**Not frozen:** Code — fixes ship when clusters earn them

---

## Remember (2026)

When we started, ARK looked like software. Today it looks like a company — not because of more features, but because every product has a job.

Platform owns truth. ARKv2 projects work. Companion projects communications. Every future product must answer: *What authority do I project?*

The development loop changed:

```text
Shop → Pressure → Observation → Notebook → Cluster → Cursor → Ship
```

The shop became the product designer. Cursor is the implementation partner.

**Protect boredom.** Great communications software disappears. If Molly reaches for Companion without thinking — and ARKv2 only for operational work — the architecture is working.

**Every interaction begins with a person and ends with work.** Go answer the phone. The advisor is the interesting part.

---

## What this is

The product tells you what it should become. This notebook captures that.

**No solutions. Only observations.**

Use Companion for real shop work — not a demo hour. When Molly answers a customer, you call back, voicemail lands, inspection photos go out, estimates get followed up, work gets approved — reach for Companion whenever it makes sense.

Every friction becomes a row here.

---

## What belongs here

| Belongs | Does not belong |
| --- | --- |
| Took three taps to call back | "Could be prettier" |
| Couldn't tell parts had arrived | Feature ideas |
| Brief missed the promise | Architecture debates |
| Opened ARKv2 just to see mileage | Solutions or fix proposals |
| Notification wasn't actionable | "We should add…" |

**Gold sentences start with:** *I wish…* · *I hesitated…* · *I reached for ARKv2 because…*

---

## Entry format

```text
YYYY-MM-DD · [who if relevant]
Observation: …
Context: … (optional — inbound call, thread reply, push, etc.)
```

Delight counts too. Note what felt inevitable.

---

## Log

<!-- Newest first. One observation per friction moment — not essays. -->

### 2026-07-06

_Observation sprint begins. Companion v1 mission frozen. Use it for everything that fits._

<!-- 
Example (delete when real entries arrive):

2026-07-08 · Edward
Observation: Took three taps to call back from a missed-call notification.
Context: Push arrived during lunch; opened Companion from lock screen.

2026-07-09 · Molly
Observation: I wish I could see mileage without opening ARKv2.
Context: Customer asked about last service interval on inbound call.
-->

---

## Friday ritual

1. Read the week's entries aloud — verbatim *I wish…* sentences matter.
2. **Cluster** — group by repeated pressure, not by screen.
3. Name the cluster in shop language (*call-back friction*, *parts visibility*, *brief missed promise*).
4. Clusters become backlog — not individual rows.
5. **No cluster → no build.** Observation earns the right to change.

| Cluster | Count | Oldest | Action |
| --- | --- | --- | --- |
| | | | |

---

## Identity (one sentence)

**ARK is an operating system for repair shops.**

**ARKv2** answers: *What work needs to be done?*  
**ARK Companion** answers: *Who needs me right now?*

Full doctrine: [`../ecosystem/ark-product-identity-v1.md`](../ecosystem/ark-product-identity-v1.md)

If a proposed change does not reinforce that split, it waits.

---

## Protected hierarchy

```
Identity → Conversation → Advisor Brief → Operational Context
```

Production feel wraps the hierarchy. It does not compete with it.

---

## Doctrine companions

- [ARK Product Identity v1](../ecosystem/ark-product-identity-v1.md) — frozen north star
- [Pressure First](../../.cursor/rules/ark-pressure-first.mdc) — observe before enforce
- [Earned Intelligence](../../.cursor/rules/ark-earned-intelligence.mdc) — repeated sentences earn features
- [Observations](../../.cursor/rules/ark-observations.mdc) — interpretive truth from floor evidence
- [09-production-feel.md](09-production-feel.md) — M7 complete; floor cert is this notebook

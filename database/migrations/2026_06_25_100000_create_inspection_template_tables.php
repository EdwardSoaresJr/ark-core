...
return new class extends Migration
{
    ...
    public function down(): void
    {
        if (Schema::hasColumn('inspection_items', 'inspection_template_item_id')) {
            Schema::table('inspection_items', function (Blueprint $table) {
                $table->dropForeign(self::INSP_ITEM_TEMPLATE_FK);
                $table->dropColumn(['inspection_template_item_id', 'checklist_category_name']);
            });
        }

        Schema::dropIfExists('inspection_template_items');
        Schema::dropIfExists('inspection_template_categories');
